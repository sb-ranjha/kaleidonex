    ...
    >
      ...
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10" ref={menuRef}>
        ...
        <AnimatePresence>
          {isMenuOpen && (
            <motion.div
              className="md:hidden fixed inset-0 top-0 left-0 right-0 bottom-0 z-[9999] bg-gradient-to-b from-primary to-purple-900 overflow-y-auto"
                ...
                {navigation.map((item, index) => (
                  ...
                  >
                    {item.dropdown ? (
                      <div className="space-y-1">
                        <button
                          ...
                          className={`flex items-center justify-between w-full text-white text-lg py-3 px-4 rounded-lg ${
                            activeDropdown === index ? 'bg-purple-600/30 border border-purple-500/30' : 'hover:bg-white/5'
                          }`}
                        >
                        ...
                        <AnimatePresence>
                          {activeDropdown === index && (
                            <motion.div
                              className="ml-6 space-y-1 bg-purple-900/30 border border-purple-800/30 rounded-xl p-4 mt-2"
                            ...
                            >
                              {item.dropdown.map((dropdownItem, idx) => (
                                ...
                                >
                                  <Link
                                    to={dropdownItem.href}
                                    className="flex items-center gap-3 text-gray-200 hover:text-white py-3 px-4 rounded-lg hover:bg-purple-600/20 transition-all duration-300 border border-transparent hover:border-purple-500/20"
                                    onClick={() => setIsMenuOpen(false)}
                                  >
                                    <span className="text-purple-400 bg-purple-900/30 p-2 rounded-lg">
                                      {dropdownItem.icon}
                                    </span>
                      ...
                      <Link
                        to={item.href}
                        className={`flex items-center gap-4 text-white text-lg py-3 px-4 rounded-lg transition-all duration-300 border border-transparent ${
                          isActivePath(item.href) ? 'bg-purple-600/30 border-purple-500/30' : 'hover:bg-white/5 hover:border-white/10'
                        }`}
                        onClick={() => setIsMenuOpen(false)}
                      >
